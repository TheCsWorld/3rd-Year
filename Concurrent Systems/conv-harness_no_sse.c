...
void team_conv_sparse(float ***image, struct sparse_matrix ***kernels, float ***output,
    ...
    // set up OpenMP
    #pragma omp parallel for if (num_kernels > 63) private(image_x, image_y, kernel_x, kernel_y, kernel_num, start_ix, end_ix, ix, channel, value, sum, cur_kernels, cur_kernel, image_row, image_col) shared(image, kernels, output) collapse(3)
