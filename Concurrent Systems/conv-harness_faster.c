                      ...
                      int width, int height, int nchannels, int nkernels, int kernel_order)
{
    ...
    else
    {
        ...
        int image_x, image_y, kernel_x, kernel_y, kernel_num, start_ix, end_ix, ix, channel;
        float value, sum;
        ...
        // set up OpenMP
        #pragma omp parallel for if (nkernels > 63) private(image_x, image_y, kernel_x, kernel_y, kernel_num, start_ix, end_ix, ix, channel, value, sum, cur_kernels, cur_kernel, image_row, image_col) shared(image, kernels, output) collapse(3)
        for (image_x = 0; image_x < width; image_x++)
        {
            ...
            for (image_y = 0; image_y < height; image_y++)
            {
                ...
                for (kernel_x = 0; kernel_x < kernel_order; kernel_x++)
                {
                    ...
                    for (kernel_y = 0; kernel_y < kernel_order; kernel_y++)
                    {
                        ...
                        for (kernel_num = 0; kernel_num < nkernels; kernel_num++)
                        {

                            // only access the start/end indices once
                            start_ix = cur_kernel->kernel_starts[kernel_num];
                            end_ix = cur_kernel->kernel_starts[kernel_num + 1];
                            ...
                            // access 'output' once
                            output[kernel_num][image_y][image_x] += sum;
